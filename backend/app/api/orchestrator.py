...
async def start_campaign(
    ...
    """
    Start a new campaign with the orchestrator. Requires authentication.
...
async def get_campaign_state(
    ...
    """Get current state of a campaign session. Requires authentication."""
...
async def select_concept(
    ...
    """
    Select a concept and continue campaign. Requires authentication.
...
async def orchestrator_websocket(
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Failed to send error message to WebSocket (client may have disconnected): {send_error}")
    finally:
