...
Your feedback is direct, constructive, and always in service of making the work better."""
    ...
    async def develop_concept(
        ...
        Args:
            ...
            asset_types: Types of assets to generate specs for. If not provided,
                         defaults are derived from channel_strategy in the brief.
            brand_data: Optional brand data for additional context

        Returns:
            Fully developed CreativeConcept
        """
        # Convert dataclass objects to dicts for uniform access
        brief = _to_dict(brief)
        territory = _to_dict(territory)

        # Determine asset types if not explicitly provided
        if asset_types is None:
            asset_types = self._derive_asset_types(brief, brand_data)
    ...
    def _derive_asset_types(self, brief, brand_data: Optional[Dict[str, Any]] = None) -> List[str]:
        """Derive default asset types from the brief's channel strategy."""
        default_types = ["instagram_post", "facebook_post", "email_header"]

        # Try to extract from channel_strategy in the brief (handles both dict and dataclass)
        channels = brief.get("channel_strategy", []) if isinstance(brief, dict) else getattr(brief, 'channel_strategy', [])
        if not channels and hasattr(brief, 'channel_strategy'):
            channels = brief.channel_strategy if brief.channel_strategy else []
        ...
        channel_to_assets = {
            ...
            "youtube": ["youtube_thumbnail", "video_ad"],
            "tiktok": ["tiktok_video"],
            "email": ["email_header"],
        ...
        derived = []
        for ch in channels:
            channel_name = ch.get("channel", "") if isinstance(ch, dict) else getattr(ch, "channel", "")
            channel_lower = channel_name.lower()
            for key, asset_list in channel_to_assets.items():
                if key in channel_lower:
                    derived.extend(asset_list)

        return derived if derived else default_types
