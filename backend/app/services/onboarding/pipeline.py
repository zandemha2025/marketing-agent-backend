...
class OnboardingPipeline:
    ...
    async def _analyze_brand(
        ...
        analysis = {
            ...
            "core_values": brand.get("core_values", []),
    ...
    async def _synthesize_data(
        ...
        self.result.brand_data = {
            ...
            "voice": {
                "tone": brand_analysis.get("brand_voice", "professional").split(", "),
                "personality": brand_analysis.get("brand_voice", ""),
                "vocabulary": brand_analysis.get("key_phrases", [])[:10],
                "avoid": [],
                "sample_phrases": brand_analysis.get("key_phrases", [])[:5]
            },
            "values": brand_analysis.get("core_values", []),
        ...
        # Use the new products field if available, otherwise fallback to extraction
        products = getattr(crawl_result, 'products', [])
        if not products:
            extracted_offerings = self._extract_offerings(crawl_result)
            products = extracted_offerings.get('products', [])
            
        self.result.offerings_data = {
            "products": products,
            "services": [],
            "key_differentiators": []
        }
        ...
        if not self.result.brand_data.get('name'):
            self.result.brand_data['name'] = self._extract_company_name(crawl_result)
            
        # Industry validation - NEVER return "Unknown"
        industry = self.result.market_data.get('industry', '')
        if not industry or industry.lower() in ['unknown', 'n/a', 'not available', '']:
            # Try to infer from domain
            industry = self.perplexity._infer_industry_from_domain(crawl_result.domain)
            self.result.market_data['industry'] = industry
        
        # Audience validation - MUST have at least 2 segments
        if not self.result.audiences_data.get('segments') or len(self.result.audiences_data['segments']) < 2:
            # Generate default segments based on industry
            company_name = self.result.brand_data.get('name', 'Company')
            default_segments = self.perplexity._get_default_audience_segments(
                company_name, 
                self.result.market_data.get('industry')
            )
            self.result.audiences_data['segments'] = [
                {
                    "name": s.segment_name,
                    "size": "primary" if i == 0 else "secondary",
                    "demographics": s.demographics,
                ...
                for i, s in enumerate(default_segments[:3])  # Max 3 segments
            ]
        
        # Offerings validation - MUST have at least 3 products/services
        offerings = self.result.offerings_data
        total_offerings = len(offerings.get('products', [])) + len(offerings.get('services', []))
        if total_offerings < 3:
            # Extract more from crawled data
            additional = self._extract_offerings(crawl_result)
            if additional.get('products'):
                existing_names = {p['name'] for p in offerings.get('products', [])}
                for prod in additional['products']:
                    if prod['name'] not in existing_names:
                        offerings['products'].append(prod)
                        existing_names.add(prod['name'])
            
            # If still less than 3, add generic placeholders based on industry
            if len(offerings.get('products', [])) + len(offerings.get('services', [])) < 3:
                industry = self.result.market_data.get('industry', '').lower()
                company_name = self.result.brand_data.get('name', 'Company')
                
                if 'fintech' in industry or 'financial' in industry or 'payment' in industry:
                    offerings['products'].extend([
                        {"name": "Payment Processing", "description": f"Core payment infrastructure for {company_name}", "url": ""},
                        {"name": "Financial Analytics", "description": "Real-time financial insights and reporting", "url": ""},
                        {"name": "API Platform", "description": "Developer tools and integration APIs", "url": ""}
                    ])
                elif 'tech' in industry or 'software' in industry or 'saas' in industry:
                    offerings['products'].extend([
                        {"name": "Core Platform", "description": f"Main software platform from {company_name}", "url": ""},
                        {"name": "Enterprise Solution", "description": "Scalable solution for large organizations", "url": ""},
                        {"name": "Integration Tools", "description": "Connectors and APIs for ecosystem integration", "url": ""}
                    ])
                else:
                    offerings['products'].extend([
                        {"name": "Core Product", "description": f"Primary offering from {company_name}", "url": ""},
                        {"name": "Premium Solution", "description": "Enhanced version with additional features", "url": ""},
                        {"name": "Support Services", "description": "Professional services and support", "url": ""}
                    ])
        ...
        # Build comprehensive Brand DNA
        self.result.brand_dna = {
            "heritage": getattr(market_research, 'brand_heritage', ""),
            "cultural_impact": getattr(market_research, 'cultural_impact', ""),
            "advertising_strategy": getattr(market_research, 'advertising_strategy', ""),
            "citations": getattr(market_research, 'citations', []),
            "core_identity": {
        ...
        await self._update_progress(
            OnboardingStage.SYNTHESIZING,
            0.9,
            "Research synthesized into Brand DNA",
            on_progress
        )
    ...
    def _extract_offerings(self, crawl_result: CrawlResult) -> Dict[str, Any]:
        """Extract products/services from crawled pages with enhanced detection."""
        products = []
        services = []
        seen_names = set()  # Track to avoid duplicates

        # First, use the products extracted from Firecrawl if available
        if hasattr(crawl_result, 'products') and crawl_result.products:
        ...
        # Then check for product pages
        for page in crawl_result.pages:
            if page.page_type == "product":
                name = page.title.strip() if page.title else ''
                # Skip if already added or if name is empty/generic
                if name and name not in seen_names and len(name) > 2:
                    seen_names.add(name)
                    products.append({
                        "name": name,
                        "description": page.description[:200] if page.description else '',
                        "url": page.url
                    })

        # Look for services on about/services pages
        for page in crawl_result.pages:
            if any(keyword in page.url.lower() for keyword in ['service', 'solution', 'offering', 'what-we-do']):
                # Extract service-like content
                if page.headings:
                    for heading in page.headings[:5]:  # Check first 5 headings
                        heading_lower = heading.lower()
                        if any(word in heading_lower for word in ['service', 'solution', 'consulting', 'support', 'management']):
        ...
        # Ensure we have at least some offerings - extract from any page if needed
        if not products and not services:
            # Try to extract from home page content
            home_page = next((p for p in crawl_result.pages if p.page_type == "home"), None)
            if home_page and home_page.headings:
                for heading in home_page.headings[:3]:
                    if heading not in seen_names and len(heading) > 3:
                        seen_names.add(heading)
                        products.append({
                            "name": heading,
                            "description": "",
